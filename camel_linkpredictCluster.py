...
#author: alex sun
#date: 01/18/2021
#desc: link prediction on Data
#this script does the following
#collect static attributes from Data dataset
...
from readDataClusters import getStaticAttr,getSubSet4Clustering,getSubSet
...
def genAdjacency(featureMat, perc=99,smeasure='euclidean'):
...
def train(df, reTrain=False, latent_dim  = 5):
    """Train autoencoder
    Parameters
    ---------
    ...
    model.to(device)
    #model_path = f'models/Data_ae_{latent_dim}.pth'
    model_path = f'Data_ae_{latent_dim}.pth'
...
def plotStations(allDF,cvec,n_clusters):
    ...
    plt.savefig('Data_cluster.png')
    plt.close()

def drawGraph(G, pos, nodelist=None,nodeval=None,allDF=None,prefix='Data_graph'):
    ...
    if nodelist is None:
        ...
        for inode in nodelist:
            ...
            netx.draw(G, pos, ax=ax, nodelist=alist, node_color=nodecolors, 
                    edgelist=elist,node_size=20, vmin=-1.0, vmax=1.0)
...
def plotDegreeHist(A,allDF,nNeighbors=4, **kwargs):
    cutoff = kwargs['cutoff']
    latenttype=kwargs['latenttype']
    smeasure=kwargs['similarity']
    #undirected graph
...
def findBadNeighbors(allDF, A, **kwargs):
    ...
    if baseline:
        ...
        for ix, item in enumerate(res.keys()): 
            dflstm = res[item]
            nselstm[ix] = Hydrostats.r_squared(dflstm['qobs'], dflstm['qsim']) 
    else:
        ...
        nselstm = pkl.load(open('gwnet_r2_statics_PredictorsClusterTopoX9_seq6_euclidean_cutoff99_latenttypefull_hiddensize13_seed2251819_log.pkl', 'rb'))        
    ...
    for inode in nodelist:
        ...
        for n in G.neighbors(inode):
            ...
            nn_r.append(nselstm[n])
        if nselstm[inode]<0.0 and np.mean(nn_r)<0.0:
            print (f"bad node {inode}, {allDF.loc[allDF.index[inode], ['gauge_str']]}, node r2 {nselstm[inode]:.2f}, mean r2 {np.mean(nn_r):.2f}")
    ...
    def getStats(runs, modelname='klstm'):
        ...
        if modelname=='klstm':
            for runitem in runs:
                ...
                for ix, item in enumerate(res.keys()): 
                    dflstm = res[item]
                    nselstm[ix] = Hydrostats.r_squared(dflstm['qsim'],dflstm['qobs']) 
                #get stat
                meanRSQ.append(np.mean(nselstm))
                medianRSQ.append(np.median(nselstm))
                minRSQ.append(np.min(nselstm))
                maxRSQ.append(np.max(nselstm))
                print (runitem)
                print (f"""mean r2 {np.mean(nselstm):.3f}, median {np.median(nselstm):.3f},
                    minRSQ {np.min(nselstm):.3f}, maxRSQ {np.max(nselstm):.3f}"""
                )
        elif modelname=='gwt':
            for seed in runs:
                ...
                nselstm=pkl.load(open('gwnet_r2_statics_PredictorsClusterTopoX9_seq6_euclidean_cutoff99_latenttypefull_hiddensize13_seed{0}_log.pkl'.format(seed),'rb'))
                #get stat
                meanRSQ.append(np.mean(nselstm))
                medianRSQ.append(np.median(nselstm))
                minRSQ.append(np.min(nselstm))
            ...
            for seed in runs:
                ...
                nselstm=pkl.load(open('impwnetbestmodel_PredictorsClusterTopoX9_seq6_full_latent5_euclidean_cut99.0_seed{0}_nu400_statics_log.pth'.format(seed),'rb'))
                #get stat
                meanRSQ.append(np.mean(nselstm))
                medianRSQ.append(np.median(nselstm))
                minRSQ.append(np.min(nselstm))
        ...
        print (meanRSQ)
        print (medianRSQ)
        print (f'mean RSQ {np.mean(meanRSQ)}')
        print (f'median RSQ {np.mean(medianRSQ)}')
        print (f'mean minRSQ {np.mean(minRSQ)}')
        print (f'mean maxRSQ {np.mean(maxRSQ)}')  
...
def main():
    ...
    np.save(f'latentMat_dim{latent_dim}_v{version}',latentMat)
    A,D = genAdjacency(latentMat,perc=pnt_cutoff, smeasure=smeasure)
    G,pos = genGraph(A, allDF)
    #drawGraph(G,pos, nodelist=[288, 303, 342, 385, 410])

    #save the adjacency matrix
    #filename = f'data/Data_adaj_{smeasure}_latent{latent_dim}_cutoff{pnt_cutoff}.npz'
    filename = f'Data_adaj_{smeasure}_latent{latent_dim}_cutoff{pnt_cutoff}.npz'
    filename1 = f'Data_edge_{smeasure}_latent{latent_dim}_cutoff{pnt_cutoff}.npz'
    #filename1 = f'data/Data_edge_{smeasure}_latent{latent_dim}_cutoff{pnt_cutoff}.npz'
