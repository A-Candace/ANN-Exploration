...
from readDataClusters import getStaticAttr
...
#os.chdir('C:/Users/cagon/OneDrive/Desktop/Columbia/Data_Jupyter_Download')
...
def get_shared_arg_parser():
    ...
    parser.add_argument('--nhid',type=int,default=11,help='')
    parser.add_argument('--in_dim',type=int,default=11,help='inputs dimension')
...
def getDataLoaders(batchsize,seq,forcingType,latentType,addStatics=False,uselog=False):
    if uselog:
        from readDataClusters_log import genLSTMDataSets
    else:
        from readDataClusters import genLSTMDataSets
...
def test(args,model,testLoader,df,regen):
    ...
    if regen:
        ...
        if uselog:
            myscaler = pkl.load(open(f'Data_forcing_scaler_{forcingType}_log.pkl', 'rb'))  
        else:
            myscaler = pkl.load(open(f'Data_forcing_scaler_{forcingType}.pkl', 'rb'))  
    ...
    for i in range(nNode):
            ...
            df = df.dropna()            
            nse[i]= Hydrostats.r_squared(df['qsim'],df['qobs'])
            l.append(nse[i])
            
    print (f'r_squared {nse}, all rsq {*l,}')
...
def train(args, trainLoader, valLoader, A, in_dim, save_path, 
    ...
    model = GWNet(device, 
        ...
        kernel_size=1) #must change kernel_size to 4 to make this work
    ...
    if addStatics:
        if uselog:
            ...
            model_path ='/'.join([save_path,f'gwnet2finalmodel_{basestr}_statics.pth'])
...
def main():
    ...
    parser.add_argument("--nepoch", type=int, default=50, help="set the number of epochs")
    parser.add_argument("--learnrate",type=float,default=0.01, help="learning rate")
    parser.add_argument("--addstatics",action='store_true', default=True, help="true to include static attributes")
    parser.add_argument("--runtesting",action='store_true', default=False, help="true to run test")
    parser.add_argument("--forcingtype",type=str,default="PredictorsNoCB", help="forcing data to use")
    ...
    parser.add_argument("--netlatent",type=int,default=5, help="hidden dim of camel")
    parser.add_argument('--hiddensize', type=int, default=12, help='hidden size')
    ...
    seq = opt.seq_length
    latentsize=opt.netlatent #latent dim used for Data, this is legacy parameter
    ...
    adjfile = os.path.join(save_path, f'Data_adaj_{smeasure}_latent{latentsize}_cutoff{netcutoff}.npz')
    edgefile = os.path.join(save_path, f'Data_edge_{smeasure}_latent{latentsize}_cutoff{netcutoff}.npz')
    ...
    if opt.runtesting:
        ...
        if opt.uselog:
            ...
            pkl.dump([trueMat,simMat], open(f'gwnet2_{opt.seed}_seq{seq}_cutoff{netcutoff}_log.pkl', 'wb'))
        else:
            #pkl.dump([trueMat,simMat], open(f'gwnet2res/gwnet2_{opt.seed}_seq{seq}_cutoff{netcutoff}.pkl', 'wb'))
            pkl.dump([trueMat,simMat], open(f'gwnet2_{opt.seed}_seq{seq}_cutoff{netcutoff}.pkl', 'wb'))
